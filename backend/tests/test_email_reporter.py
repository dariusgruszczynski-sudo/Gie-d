...
def test_build_report_renders_both_portfolios(db_session, settings):
    ...
    db_session.add(
        Decision(
            ...
            triggered_by=TriggerType.MANUAL, executed=True, venue="extended",
        )
    )
    db_session.commit()

    html, chart_png = email_reporter.build_report(db_session, extended_settings)
...
def test_build_report_handles_extended_disabled(db_session, settings):
    ...
    html, _ = email_reporter.build_report(db_session, extended_disabled)
    assert "$500.00" in html
