...
def test_crypto_venue_folds_in_aggressive_overrides(settings):
    """The crypto venue's effective settings must take the crypto_* override
    values on the base fields, leaving the base object itself unmodified."""
    s = settings.model_copy(update={
        "risk_per_trade_pct": 1.25,
        "crypto_risk_per_trade_pct": 2.0,
        "max_concurrent_positions": 6,
        "crypto_max_concurrent_positions": 9,
        "min_buy_confidence": 0.57,
        "crypto_min_buy_confidence": 0.5,
        "min_hold_minutes": 30,
        "crypto_min_hold_minutes": 15,
    })
    ...
    # Base object untouched (crypto profile is a copy).
