...
def main():
    ...
    try:
        ...
        print(f"  min_buy_confidence (baza) = {s.min_buy_confidence}")
        print(f"  progresja pewności        = +{s.progressive_confidence_step}/pozycję, sufit {s.progressive_confidence_cap}")
        print(f"  min_hold_minutes          = {s.min_hold_minutes}  (furtka zysku od +{s.min_hold_profit_bypass_pct}%)")
        print(f"  hard_take_profit_pct      = {s.hard_take_profit_pct}  (kasuj mocny ruch)")
