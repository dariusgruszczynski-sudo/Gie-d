...
function renderInbox(view, actions) {
  ...
  // Wspólne: z wyniku detekcji buduje piosenkę i otwiera edytor.
  ...
  function detectFromFile(meta) {
    ...
    inp.addEventListener('change', async () => {
      ...
      if (!res.ok) { detectError(res.error); return; }
      applyDetected(res, { title: meta.title || file.name.replace(/\.[^.]+$/, ''), artist: meta.artist || '', notes: meta.notes || ('Plik: ' + file.name), inboxId: meta.inboxId });
    });
